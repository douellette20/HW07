	...
	void update(int clock) 
	{
		...
		if (!the_queue.empty()) {
			...
			if ((clock - plane->start_service_time) > plane->service_time) {  
				// DONE: remove plane from the service queue
				the_queue.pop();

				// DONE: update the enter_departure_time attribute for the plane
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				// DONE: calculate the wait time of the plane in the landing queue
				int wait_time = clock - plane->arrival_time;

				// DONE: update total_wait and num_served for the landing queue
				landing_queue->total_wait += wait_time;
				landing_queue->num_served += 1;

				// DONE: update the start_service_time attribute for the plane
				plane->start_service_time = clock;

				/* DONE: compute a random service time for the plane between min_service_time and max_service_time
				...
				*/
				...
				// DONE: add the plane to the service queue
